...
export function GuildChatClient({ guildId }: { guildId: string }) {
  ...
  const fetchMessages = async () => {
    const res = await fetch(`/api/chat/${guildId}?channel=${activeChannel}`)
    if (res.ok) {
  ...
  const setupPusher = async () => {
    ...
    const channelName = `guild-${guildId}-${activeChannel}`
